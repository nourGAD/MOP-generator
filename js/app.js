...
function updCut(){
  if(TRMm()){ const ar=S.types.map(t=>{const r=M.schedule(S,t);return r.days.find(d=>d.cut).arrive}); $("#cutLbl").firstChild.textContent="Team on site – outage day";
    $("#cutOut").textContent=ar.length?M.clock(Math.min(...ar))+(Math.min(...ar)!==Math.max(...ar)?" – "+M.clock(Math.max(...ar)):""):"–"; $("#cutLbl small").textContent="auto: just in time for the outage (per link type)"; return; }
  $("#cutLbl").firstChild.textContent=`Team on site – cutover (Day ${M.cutDayOf(S.project)})`; $("#cutOut").textContent=M.clock(M.cutArrive(S));
  ...
  $("#types").innerHTML=S.types.map((t,i)=>`
   <div class="tcard" style="--c:${COL(i)}" data-i="${i}">
    ...
    ${TRMm()?`<div class="opts"><label class="f">Swap method<select data-t="method">${Object.entries(M.TRM_METHODS).map(([k,v])=>`<option value="${k}" ${t.method===k?"selected":""}>${esc(v.label)}</option>`).join("")}</select></label>
      ${t.method==="normal"?`<label class="chk"><input type="checkbox" data-t="compact" ${t.compact?"checked":""}> Install both ends on Day 1 (small antennas)</label>`:""}
...
function render(){
  ...
  $("#sum").innerHTML=S.types.map((t,i)=>{const r=sch[i],bad=Math.max(...r.days.map(d=>d.hours))>mx+0.5;
    return `<button class="tile" style="--c:${COL(i)}" data-sel="${i}" aria-pressed="${i===sel}">
      ...
      <div class="eqt">${TRMm()?`<b>${esc((M.TRM_METHODS[t.method]||M.TRM_METHODS.normal).label)}${t.sd?" + SD":""}</b> · `:""}${esc(M.equipTxt(t))}</div>
      <dl class="${r.days.length>=3?"many":""}" style="grid-template-columns:repeat(${r.days.length+1},1fr)">${r.days.map((d,k)=>`<div><dt>Day ${k+1}${d.cut?(TRMm()?" 🔴":" ⚡"):""}</dt><dd>${M.fmt(d.hours)}</dd></div>`).join("")}<div><dt>Outage</dt><dd class="o">${M.fmt(r.outage)}</dd></div></dl>
  ...
  $("#tl").innerHTML=`<h2>${esc(t.name)} — ${r.days.length} day${r.days.length>1?"s":""} ${TRMm()?"per link":"on site"}</h2>
    ...
    ${r.days.map(day).join("")}
    <div class="note">The Excel file has the full MOP for every ${TRMm()?"link":"site"} type: steps, who, dependencies, clock times, time mapping, connections and a ${TRMm()?"link":"site"} tracker with dates.</div>`;
}
...
document.addEventListener("input",e=>{
  ...
  else if(el.dataset.t){const i=+el.closest(".tcard").dataset.i; S.types[i][el.dataset.t]=v; if(el.dataset.t==="method"){renderInputs()} updCut()}
