...
if(!S||!S.types) S=M.DEFAULT_STATE();
const D=M.DEFAULT_STATE(); S.project=Object.assign({},D.project,S.project||{}); S.fixed=Object.assign({},D.fixed,S.fixed||{}); S.unit=Object.assign({},D.unit,S.unit||{});
let sel=0;
...
function renderInputs(){
  ...
  $("#fixed").innerHTML=M.FIXED.map(([k,l])=>`<div class="fx"><span>${esc(M.sub(S,l))}</span><input type="number" min="0" data-f="${k}" value="${S.fixed[k]}"></div>`).join("");
}
...
function render(){
  ...
  $("#sum").innerHTML=S.types.map((t,i)=>{const r=sch[i],bad=Math.max(r.days[0].hours,r.days[1].hours)>mx+0.5;
    return `<button class="tile" style="--c:${COL(i)}" data-sel="${i}" aria-pressed="${i===sel}">
      ...
      <dl><div><dt>Day 1</dt><dd>${M.fmt(r.days[0].hours)}</dd></div><div><dt>Day 2</dt><dd>${M.fmt(r.days[1].hours)}</dd></div><div><dt>Outage</dt><dd class="o">${M.fmt(r.outage)}</dd></div></dl>
      <span class="flag ${bad?"bad":""}">${bad?"⚠ Over "+S.project.maxHours+" h – split or add crew":"✔ Fits the working day"}</span>
    </button>`}).join("");
  // totals
  ...
  const day=(d,di)=>{const bad=d.hours>mx+0.5;
    const ob=di===1?`<div class="ob" style="left:${(r.outStart-d.arrive)/W*100}%;width:${r.outage/W*100}%"></div>`:"";
  ...
  $("#tl").innerHTML=`<h2>${esc(t.name)} — two days on site</h2>
    <div class="sub">Outage ${M.clock(r.outStart)} → ${M.clock(r.outEnd)} (${M.fmt(r.outage)}) · Total ${M.fmt(r.total)} on site${t.reuseCabling?" · cable runs skipped (reused)":""}</div>
    <div class="legend"><span style="--k:var(--amber)">Tower crew</span><span style="--k:var(--blue)">Ground / FE</span><span style="--k:var(--violet)">Remote integrator</span><span style="--k:var(--red)">Outage</span></div>
    ${r.days.map(day).join("")}
...
document.addEventListener("input",e=>{
  const el=e.target, v=el.type==="number"?(el.value===""?0:+el.value):el.type==="checkbox"?el.checked:el.value;
  if(el.id==="pname") S.project.name=v;
    ...
    if(["newBB","oldBB"].includes(el.dataset.p)){clearTimeout(window._rf);window._rf=setTimeout(()=>{$("#fixed").innerHTML=M.FIXED.map(([k,l])=>`<div class="fx"><span>${esc(M.sub(S,l))}</span><input type="number" min="0" data-f="${k}" value="${S.fixed[k]}"></div>`).join("")},400)} }
  else if(el.dataset.u) S.unit[el.dataset.u]=v;
  else if(el.dataset.f) S.fixed[el.dataset.f]=v;
...
$("#imp").onchange=async e=>{
  ...
  try{const x=JSON.parse(await f.text()); if(!x||!Array.isArray(x.types)||!x.project) throw new Error("not a MOP setup file");
    const d=M.DEFAULT_STATE(); S=Object.assign(d,x); S.project=Object.assign({},d.project,x.project); S.fixed=Object.assign({},d.fixed,x.fixed||{}); S.unit=Object.assign({},d.unit,x.unit||{});
